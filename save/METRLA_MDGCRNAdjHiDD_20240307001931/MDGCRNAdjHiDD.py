...
class MDGCRNAdjHiDD(nn.Module):
    def __init__(self, num_nodes, input_dim, output_dim, horizon, rnn_units, rnn_layers=1, cheb_k=3,
        ...
        if self.use_STE:
            self.decoder = ADCRNN_Decoder(self.num_nodes, self.rnn_units + self.embed_dim * 2, self.decoder_dim, self.cheb_k, self.rnn_layers, 1+len(self.adj_mx))
        else:
            self.decoder = ADCRNN_Decoder(self.num_nodes, self.output_dim + self.ycov_dim, self.decoder_dim, self.cheb_k, self.rnn_layers, 1+len(self.adj_mx))
    ...
    def forward(self, x, x_cov, x_his, y_cov, labels=None, batches_seen=None):
        ...
        supports_de = self.adj_mx + [support] 
