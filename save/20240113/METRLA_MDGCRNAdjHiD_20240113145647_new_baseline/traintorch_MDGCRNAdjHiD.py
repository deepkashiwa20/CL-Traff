...
def get_model():
    ...
    model = MDGCRNAdjHiD(num_nodes=args.num_nodes, input_dim=args.input_dim, output_dim=args.output_dim, horizon=args.horizon, 
                 ...
                 mem_dim=args.mem_dim, embed_dim=args.embed_dim, time_dim=args.time_dim, adj_mx = adjs, cl_decay_steps=args.cl_decay_steps, use_curriculum_learning=args.use_curriculum_learning, 
...
parser.add_argument('--time_dim', type=int, default=10, help='embedding dimension for time slot')
...
parser.add_argument('--lamb2', type=float, default=0.0, help='anomaly detection loss lambda') 
parser.add_argument('--schema', type=int, default=0, choices=[0, 1, 2, 3, 4], help='which schema to implement latent distance')
