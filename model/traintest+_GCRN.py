...
def get_xs_ys_time(data, data_time, mode):
    train_num = int(data.shape[0] * args.trainval_ratio)
    xs, ys, xs_time, ys_time = [], [], [], []
    if mode == 'train':    
        for i in range(train_num - args.horizon - args.seq_len + 1):
            x = data[i:i+args.seq_len, ...]
            x_t = data_time[i:i+args.seq_len, ...]
            y = data[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            y_t = data_time[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            xs.append(x), ys.append(y), xs_time.append(x_t), ys_time.append(y_t)
    elif mode == 'test':
        for i in range(train_num - args.seq_len,  data.shape[0] - args.horizon - args.seq_len + 1):
            x = data[i:i+args.seq_len, ...]
            x_t = data_time[i:i+args.seq_len, ...]
            ...
            y_t = data_time[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            xs.append(x), ys.append(y), xs_time.append(x_t), ys_time.append(y_t)
    xs, ys, xs_time, ys_time = np.array(xs), np.array(ys), np.array(xs_time), np.array(ys_time)
    xs, ys, xs_time, ys_time = xs[:, :, :, np.newaxis], ys[:, :, :, np.newaxis], xs_time[:, :, :, np.newaxis], ys_time[:, :, :, np.newaxis]
    return xs, ys, xs_time, ys_time
...
def evaluate_model(model, data_iter):
    ...
    with torch.no_grad():
        for x, y, xcov, ycov in data_iter:
            y_pred, _ = model(x, xcov, ycov)
...
def train_model(name, mode, xs, ys, xcov, ycov):
    model = get_model(mode)
    
    xs = scaler.transform(xs)
    xs_torch, ys_torch, xcov_torch, ycov_torch = torch.Tensor(xs).to(device), torch.Tensor(ys).to(device), torch.Tensor(xcov).to(device), torch.Tensor(ycov).to(device)
    trainval_data = torch.utils.data.TensorDataset(xs_torch, ys_torch, xcov_torch, ycov_torch)
...
def test_model(name, mode, xs, ys, xcov, ycov):
    model = get_model(mode)
    model.load_state_dict(torch.load(modelpt_path))
    
    xs = scaler.transform(xs)
    xs_torch, ys_torch, xcov_torch, ycov_torch = torch.Tensor(xs).to(device), torch.Tensor(ys).to(device), torch.Tensor(xcov).to(device), torch.Tensor(ycov).to(device)
    test_data = torch.utils.data.TensorDataset(xs_torch, ys_torch, xcov_torch, ycov_torch)
...
def main():
    ...
    train_xs, train_ys, train_xcov, train_ycov = get_xs_ys_time(data, data_time, 'train')
    logger.info('Train xs.shape ys.shape, xcov.shape, ycov.shape', train_xs.shape, train_ys.shape, train_xcov.shape, train_ycov.shape)
    train_model(model_name, 'train', train_xs, train_ys, train_xcov, train_ycov)
    logger.info(args.dataset, 'training ended', time.ctime())
    
    test_xs, test_ys, test_xcov, test_ycov = get_xs_ys_time(data, data_time, 'test')
    logger.info('Test xs.shape, ys.shape, test_xcov.shape, ycov.shape', test_xs.shape, test_ys.shape, test_xcov.shape, test_ycov.shape)
    test_model(model_name, 'test', test_xs, test_ys, test_xcov, test_ycov)
