...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            output, h_att, query, pos, neg, mask, real_dis, latent_dis = model(x, x_cov, scaler.transform(x_his), y_cov)
...
def traintest_model():  
    ...
    for epoch_num in range(args.epochs):
        ...
        for x, y in data_iter:
            optimizer.zero_grad()
            x, x_cov, x_his, y, y_cov, _ = prepare_x_y(x, y)
            output, h_att, query, pos, neg, mask, real_dis, latent_dis = model(x, x_cov, scaler.transform(x_his), y_cov, scaler.transform(y), batches_seen)
            y_pred = scaler.inverse_transform(output)
            y_true = y
