...
class MDGCRNAdj(nn.Module):
    ...
    def construct_memory(self):
        ...
        memory_dict['Memory_c'] = nn.Parameter(torch.randn(self.mem_num, self.mem_dim), requires_grad=True)     # (M, d) for contrast
    ...
    def query_memory(self, h_t:torch.Tensor):
        ...
        pos = self.memory['Memory_c'][ind[:, :, 0]] # B, N, d
        if self.contra_loss in ['infonce']:  # InfoNCE loss
            neg = self.memory['Memory_c'].repeat(query.shape[0], self.num_nodes, 1, 1)  # (B, N, M, d)
            ...
            neg = self.memory['Memory_c'][ind[:, :, 1]] # B, N, d
            mask = None
        else:
            pass
