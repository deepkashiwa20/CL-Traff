...
class ContrastiveLoss():
    ...
    def calculate(self, query, pos, neg, mask, att_score):
        """
        ...
        if not self.infonce:
            ...
            # triplet loss
            separate_loss = nn.TripletMarginLoss(margin=0.2)  # TODO: set hyperparameter
            u_loss2 = separate_loss(query, pos.detach(), neg.detach()) * 1.0 # TODO: set hyperparameter
            u_loss += u_loss2
            return u_loss  
...
def get_model():
    ...
    model = MDGCRNAdj(num_nodes=args.num_nodes, input_dim=args.input_dim, output_dim=args.output_dim, horizon=args.horizon, 
                 ...
                 use_curriculum_learning=args.use_curriculum_learning, contra_loss=args.contra_loss, pos_n=args.pos_n, device=device).to(device)
    return model
...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            output, h_att, query, pos, neg, mask, _ = model(x, x_cov, y_cov)
...
def traintest_model():  
    ...
    for epoch_num in range(args.epochs):
        ...
        for x, y in data_iter:
            optimizer.zero_grad()
            x, x_cov, y, y_cov = prepare_x_y(x, y)
            output, h_att, query, pos, neg, mask, att_score = model(x, x_cov, y_cov, scaler.transform(y), batches_seen)
            y_pred = scaler.inverse_transform(output)
            y_true = y
            ...
            u_loss = separate_loss.calculate(query, pos, neg, mask, att_score)
...
parser.add_argument('--pos_n', type=int, default=3, help='number of positive samples')
