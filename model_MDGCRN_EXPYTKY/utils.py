...
def load_npy(npy_file, num_nodes, normalized_k=0.9999997):
    adj = np.load(npy_file)     #* symmetric matrix
    adj = adj[:num_nodes, :num_nodes]
    if adj.max() > 1:           # distance adj instead 0/1 adj
...
def load_adj(pkl_filename, adjtype, num_nodes):
    if pkl_filename.endswith("pkl"):
        sensor_ids, sensor_id_to_ind, adj_mx = load_pickle(pkl_filename)
    elif pkl_filename.endswith("npy"):
        adj_mx = load_npy(pkl_filename, num_nodes)
    if adjtype == "scalap":
