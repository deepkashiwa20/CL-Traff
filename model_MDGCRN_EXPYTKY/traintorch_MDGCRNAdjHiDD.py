...
def get_model():
    adj_mx = load_adj(adj_mx_path, args.adj_type, args.num_nodes)
...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        if mode == 'test':
            ...
            mae_1 = masked_mae_loss(ys_pred[:, 0, ...], ys_true[:, 0, ...]).item()
            mape_1 = masked_mape_loss(ys_pred[:, 0, ...], ys_true[:, 0, ...]).item()
            rmse_1 = masked_rmse_loss(ys_pred[:, 0, ...], ys_true[:, 0, ...]).item()
            
            mae_2 = masked_mae_loss(ys_pred[:, 1, ...], ys_true[:, 1, ...]).item()
            mape_2 = masked_mape_loss(ys_pred[:, 1, ...], ys_true[:, 1, ...]).item()
            rmse_2 = masked_rmse_loss(ys_pred[:, 1, ...], ys_true[:, 1, ...]).item()
            ...
            mae_4 = masked_mae_loss(ys_pred[:, 3, ...], ys_true[:, 3, ...]).item()
            mape_4 = masked_mape_loss(ys_pred[:, 3, ...], ys_true[:, 3, ...]).item()
            rmse_4 = masked_rmse_loss(ys_pred[:, 3, ...], ys_true[:, 3, ...]).item()
            
            mae_5 = masked_mae_loss(ys_pred[:, 4, ...], ys_true[:, 4, ...]).item()
            mape_5 = masked_mape_loss(ys_pred[:, 4, ...], ys_true[:, 4, ...]).item()
            rmse_5 = masked_rmse_loss(ys_pred[:, 4, ...], ys_true[:, 4, ...]).item()
            ...
            logger.info('Horizon 10mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_1, mape_1 * 100, rmse_1))
            logger.info('Horizon 20mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_2, mape_2 * 100, rmse_2))
            logger.info('Horizon 30mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_3, mape_3 * 100, rmse_3))
            logger.info('Horizon 40mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_4, mape_4 * 100, rmse_4))
            logger.info('Horizon 50mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_5, mape_5 * 100, rmse_5))
            logger.info('Horizon 60mins: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae_6, mape_6 * 100, rmse_6))
...
# adj_mx_path = config['common']['adjdis_path'] 
adj_mx_path = config['common']['adj01_path']
...
#* max在训练验证集相同但是测试集更大, 因此max设为6, 是训练测试集向上取整的结果
#* min干脆设置为0, 因为abs的最小值必定>=0, 这样同样能合理解释, 也能归一化到[0, 1], 只不过最小值为0.07左右, 与0接近
