...
from StgclDGCRNPEMS import StgclDGCRNPEMS 
...
def get_model():  
    model = StgclDGCRNPEMS(num_nodes=args.num_nodes, input_dim=args.input_dim, output_dim=args.output_dim, horizon=args.horizon, 
...
def traintest_model():  
    ...
    for epoch_num in range(args.epochs):
        ...
        for x, y in data_iter:
            optimizer.zero_grad()
            x, x_cov, y, y_cov = prepare_x_y(x, y)
            output, u_loss = model(x, x_cov, y_cov, y, batches_seen) # TODO: add new parameters <xcov> here
            y_pred = scaler.inverse_transform(output)
            y_true = y
